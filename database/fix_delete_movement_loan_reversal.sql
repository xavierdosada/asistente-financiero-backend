-- Hotfix: al borrar un movement de pago de préstamo, revertir cuotas vía
-- loan_payment_events + loan_payment_allocations (event_id, applied_amount).
-- La versión legacy de delete_movement_with_reversal buscaba movement_id en
-- loan_payment_allocations y columnas total_due/paid en inglés; con el esquema
-- de add_auto_debt_link_from_movements.sql no hacía nada útil.
--
-- Requisitos: tablas loan_payment_events, loan_payment_allocations, función public.recalc_loan.
-- Ejecutar en Supabase SQL editor (luego el wrapper v1 sigue llamando a esta función).
